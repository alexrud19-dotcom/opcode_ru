...
export const Agents: React.FC = () => {
  ...
  const handleRunAgent = async (agent: Agent) => {
    if (!agent.id) {
      setToast({ message: t('agents.idMissing'), type: 'error' });
      return;
    }
  ...
  const handleImportFromFile = async () => {
    try {
      const selected = await openDialog({
        filters: [
          { name: t('agents.fileFilter'), extensions: ['opcode.json', 'json'] },
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      const path = await save({
        ...
        filters: [
          { name: t('agents.fileFilter'), extensions: ['opcode.json'] }
        ]
      });
          ...
          <TabsContent value="agents" className="flex-1 overflow-hidden">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">{t('agents.emptyTitleAlt')}</h3>
                  <p className="text-muted-foreground mb-4">
                    {t('agents.emptySubtitleAlt')}
                  </p>
                  <Button onClick={() => setShowCreateAgent(true)}>
                    <Plus className="w-4 h-4 mr-2" />
                    {t('agents.create')}
              ...
              {runningAgents.length === 0 ? (
                <Card className="p-12">
                  <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-lg font-semibold mb-2">{t('agents.historyEmpty')}</h3>
                    <p className="text-muted-foreground">
                      {t('agents.historyEmptyHint')}
                    </p>
                  </div>
                </Card>
              ) : (
